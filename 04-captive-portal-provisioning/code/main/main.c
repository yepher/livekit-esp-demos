/*
 * Captive Portal Provisioning — Main Application
 *
 * Boot flow:
 ...
 *   8. Main loop (keep-alive log)
 *
 * Unlike Posts 01–02, there are no hardcoded credentials in sdkconfig.
 * Everything is entered through the captive portal or pre-loaded via SD card.
 */
...
void app_main(void)
{
    ...
    while (1) {
        vTaskDelay(pdMS_TO_TICKS(30000));
        ESP_LOGI(TAG, "Running (heap: %lu bytes free)",
